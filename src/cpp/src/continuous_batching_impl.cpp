...
void ContinuousBatchingPipeline::ContinuousBatchingImpl::init(
    ...
    if (is_use_cache_eviction) {
        ...
        m_cache_rotation_calculator = std::make_shared<CacheRotationCalculator>(
            ...
            max_sequence_length,
            embedding_size);
        m_rotation_trig_lut = ov::Tensor(ov::element::f32, ov::Shape{max_sequence_length, device_config.get_head_size()});
        float* rotation_trig_lut_data = m_rotation_trig_lut.data<float>();
        std::memset(rotation_trig_lut_data, 0, m_rotation_trig_lut.get_byte_size());
