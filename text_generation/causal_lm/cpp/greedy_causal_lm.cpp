...
try {
    if (argc != 5) {
        throw std::runtime_error(std::string { "Usage: " } + argv[0] + " <MODEL_DIR> <GGUF_FILE> '<PROMPT>' ");
    }
    // Compile models
    ov::Core core;
    ...
    ov::InferRequest tokenizer = core.compile_model(
                                         std::string { argv[1] } + "/openvino_tokenizer.xml", "CPU")
    ...
    //
    //
    int64_t* input_data = input_ids.data<int64_t>();
    ...
    std::cout << "Input tokens re-generated:\n";
    for (size_t i = 0; i < input_ids.get_size(); i++) {
        float* regen_logits = lm.get_tensor("logits").data<float>() + (i)*vocab_size;
        int64_t regen_out_token = std::max_element(logits, logits + vocab_size) - logits;
        std::cout << regen_out_token << ' ';
    }
    std::cout << '\n';
    ...
    while (out_token != SPECIAL_EOS_TOKEN && cnt < hard_stop_token_count) {
        ...
        std::cout << "position id is " << int64_t(lm.get_tensor("attention_mask").get_size() - 2) << "\n";
